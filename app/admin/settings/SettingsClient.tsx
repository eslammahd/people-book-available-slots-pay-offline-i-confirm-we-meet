...
export default function SettingsClient({ slots: initial }: { slots: Slot[] }) {
  ...
  async function toggleAvailability(slotId: string, current: boolean) {
    ...
    if (!error) {
      setSlots((prev) =>
        prev.map((s) => s.id === slotId ? { ...s, is_available: !current } : s)
      )
    }
    setToggling(null)
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center">
        ...
        <Link href="/admin/dashboard" className="text-sm text-sky-600 hover:text-sky-700 px-3 py-1.5 rounded-lg hover:bg-sky-50">
          ← Dashboard
        </Link>
      </header>
              ...
              <div key={date}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {grouped[date].map((slot) => (
                    ...
                    >
                      <div className={`text-lg font-bold ${slot.is_available ? 'text-slate-800' : 'text-slate-400'}`}>
                        {slot.slot_time}
                      </div>
