...
type Slot = { id: string; slot_date: string; slot_time: string; duration_minutes: number; is_available: boolean }
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        {dates.length === 0 ? (
          ...
          <div className="space-y-8">
            {dates.map((date) => (
              <div key={date}>
                <h2 className="text-sm font-semibold text-sky-600 uppercase tracking-wider mb-3">{formatDate(date)}</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {grouped[date].map((slot) => (
                    <button key={slot.id} onClick={() => setSelectedSlot(slot)}
                      className="bg-white border-2 border-sky-200 hover:border-sky-500 hover:bg-sky-50 rounded-xl p-4 text-left transition-all">
