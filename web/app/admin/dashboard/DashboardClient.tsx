...
type Booking = { id: string; patient_name: string; patient_email: string; patient_phone: string; payment_status: string; created_at: string; slots: Slot }

const STATUS_CYCLE: Record<string, string> = { pending: 'paid', paid: 'complete', complete: 'pending' }
...
export default function DashboardClient({ bookings: initial }: { bookings: Booking[] }) {
  ...
  async function toggleStatus(id: string, current: string) {
    ...
    setUpdating(id)
    const { error } = await supabase.from('bookings').update({ payment_status: next }).eq('id', id)
    if (!error) setBookings((prev) => prev.map((b) => b.id === id ? { ...b, payment_status: next } : b))
    setUpdating(null)
  }
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <Link href="/admin/settings" className="text-sm text-slate-500 hover:text-slate-700 px-3 py-1.5 rounded-lg hover:bg-slate-100">Settings</Link>
          <button onClick={handleLogout} className="text-sm text-red-500 hover:text-red-700 px-3 py-1.5 rounded-lg hover:bg-red-50">Sign Out</button>
        </div>
      </header>
                  ...
                  {grouped[date].map((booking) => (
                    <div key={booking.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <button onClick={() => toggleStatus(booking.id, booking.payment_status)} disabled={updating === booking.id}
